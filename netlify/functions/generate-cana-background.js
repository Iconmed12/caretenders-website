...
exports.handler = async (event) => {
  ...
  try {
    ...
    var sharedContext =
      ...
      (localResearch
        ? 'The following local partners have been researched from current web sources for THIS area. These are REAL, verified organisation names, so you may name them directly and confidently (no [CONFIRM] flag needed for names that appear here). Weave them in naturally where the question invites partnership working, discharge, community health, safeguarding or social value, always tied to a concrete example:\n' + localResearch + '\n\n'
      ...
      '═══ EXISTING vs FUTURE LOCAL PARTNERSHIPS (do not over-claim) ═══\n' +
      'The company already operates in these areas: "' + (co.regions || 'see the company evidence below') + '". Only describe a local partnership, a local workforce, or a local presence as something that ALREADY EXISTS where this tender\'s area clearly falls within those existing operating regions. If this tender\'s area is NOT within the company\'s existing regions (the company would be expanding into it), you MUST still name the real local organisations above, but frame them as relationships you WILL establish on award, using intent language ("on award we will build relationships with", "we will establish referral pathways with", "our partnership approach will connect us with"). In that case do NOT claim existing local partnerships, do NOT use a heading or phrasing such as "existing partnerships in the area", and do NOT claim the workforce already reflects or is recruited from that specific area (recruiting there is a plan, write it as one). Never state the company already operates or is established in an area it does not currently serve: that is a verifiable claim a commissioner can disprove and it is disqualifying.\n\n' +
    ...
    try {
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'CANA', bold: true, size: 20, color: '00C9E0', font: 'Arial' })],
      ...
      children.push(new Paragraph({
        children: [new TextRun({ text: 'Generated by Cana  |  hello@getcana.co.uk', size: 18, color: '999999', font: 'Arial' })],
    ...
    var attachments = [];
    if (docBase64)   attachments.push({ filename: 'Cana_Tender_Responses.docx', content: docBase64 });
    ...
    var emailHtml = '<div style="font-family:Arial,sans-serif;max-width:600px;margin:0 auto;">' +
      ...
      '</div>' +
      '<p style="color:#9ca3af;font-size:11px;text-align:center;">Cana | hello@getcana.co.uk</p>' +
      '</div></div>';
    ...
    // Send to ops, flag the order by tier so the team sees the scope at a glance
    try {
