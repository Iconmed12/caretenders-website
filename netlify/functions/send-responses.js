  ...
  try {
    ...
    var htmlBody = '<div style="font-family:Arial,sans-serif;max-width:640px;margin:0 auto;">' +
      '<div style="background:#0B1929;padding:24px;border-radius:8px 8px 0 0;">' +
        '<h1 style="color:#00C9E0;margin:0;font-size:22px;">Cana</h1>' +
      '</div>' +
    ...
    // Attached files list
    var attachedFiles = ['<strong>Cana_Tender_Responses.docx</strong>: ' + (responses ? responses.length : 0) + ' complete bid responses'];
    ...
    htmlBody += '<hr style="margin-top:32px;"><p style="color:#9ca3af;font-size:11px;text-align:center;">Generated by Cana | hello@getcana.co.uk</p>' +
      '</div></div>';

    // ── Build attachments array ──
    var attachments = [];
    if (docBase64) {
      attachments.push({ filename: 'Cana_Tender_Responses.docx', content: docBase64 });
    }
    if (includeSq && sqDocBase64) {
      attachments.push({
    ...
    if (!iconRes.ok) errors.push('Ops email: ' + JSON.stringify(await iconRes.json()));
    else console.log('Ops email sent');
